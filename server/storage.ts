...
export class DatabaseStorage {
  ...
  // Order operations
  async getOrdersByShop(shopId: number): Promise<(Order & { coffee: GreenCoffee; user: User })[]> {
    try {
      return await db
        .select({
          id: orders.id,
  ...
  async getRetailInventoriesByShop(shopId: number): Promise<(RetailInventory & { coffee: GreenCoffee })[]> {
    try {
      return await db
        .select({
